...
// Official SAMA Saudi Riyal symbol artwork (client-provided Saudi_Riyal_Symbol.svg),
// filled (not stroke-based) — width is derived from its native aspect ratio so it
// never looks squashed when only `size` (equal width/height) is requested.
const RIYAL_VIEWBOX_W = 1124.14;
const RIYAL_VIEWBOX_H = 1256.39;

/**
 * Saudi Riyal symbol — official SAMA artwork. Inherits color via currentColor;
 * scale it with font-size using em-based height (className="h-[1em] w-auto").
 */
export function Riyal({ size, className = "", ...p }: IconProps) {
  return (
    <svg
      viewBox={`0 0 ${RIYAL_VIEWBOX_W} ${RIYAL_VIEWBOX_H}`}
      width={size ? size * (RIYAL_VIEWBOX_W / RIYAL_VIEWBOX_H) : size}
      height={size}
      fill="currentColor"
      aria-hidden="true"
      className={className}
